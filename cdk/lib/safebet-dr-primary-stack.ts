...
export class SafeBetDRPrimaryStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SafeBetDRPrimaryProps) {
    ...
    /* ── 3. Secrets Manager — auto-generated DB credentials ─────────── */
    this.dbSecret = new secretsmanager.Secret(this, 'PrimaryDbSecret', {
      secretName: CONFIG.SECRET_RDS_CREDS,
      description: 'SafeBet RDS primary — admin credentials (auto-generated)',
      generateSecretString: {
        secretStringTemplate:  JSON.stringify({ username: CONFIG.DB_ADMIN_USER }),
        generateStringKey:     'password',
        excludeCharacters:     '/@"\' \\',
        passwordLength:        32,
      },
      removalPolicy: cdk.RemovalPolicy.RETAIN,  // never auto-delete credentials
    });

    /* ── 4. Supabase credentials secret (for auto-failover Lambda) ─── */
    // Stored in Secrets Manager — never hardcoded in Lambda env vars.
    // The Lambda currently reads from env vars; a future enhancement is to
    // update it to call GetSecretValue.  For demo, credentials are passed
    // via CDK context (--context supabaseDbPassword=XXX).
    const supabaseSecret = new secretsmanager.Secret(this, 'SupabaseSecret', {
      secretName:  CONFIG.SECRET_SUPABASE_CREDS,
      description: 'SafeBet Supabase DR credentials (used by auto-failover Lambda)',
      secretStringValue: cdk.SecretValue.unsafePlainText(
        JSON.stringify({
          host:     props.supabaseDbHost,
          port:     props.supabaseDbPort,
          dbname:   props.supabaseDbName,
          username: props.supabaseDbUser,
          password: props.supabaseDbPassword,
        })
      ),
    ...
    const autoFailoverRole = new iam.Role(this, 'AutoFailoverRole', {
      ...
      description: 'Execution role for safebet-auto-failover Lambda (S3→Supabase restore)',
      managedPolicies: [
