...
export async function POST(req: NextRequest) {
  ...
  if (!rateLimit(ip)) {
    Sentry.captureMessage('Rate limit exceeded', { level: 'warning', tags: { endpoint: '/api/contact', ip } });
  ...
  try {
    ...
    if (insertErr) {
      ...
      Sentry.captureException(new Error(`[contact] Insert error: ${insertErr.message}`));
