// ─── SHARED PLATFORM FOUNDATION — Evidence framework (ARCH-V4-A4) ─────────────
//
// GOVERNED SHARED SERVICE. Canonical home of the certified evidence framework,
// extracted (A4 strangler step) from lib/consumerPlatform/evidence.ts. It is a
// SHARED capability (SafeBet IQ now; Guardian + Regulator Suite later), not an
// IQ-internal one.
//   Owner:      Shared Platform Foundation
//   Consumers:  SafeBet IQ (evidence-gateway, regulator export); Guardian/Regulator (future)
//   Kind:       PURE functions only (no DB, no side effects, no credentials)
//   Auth/scope: the CALLER derives scope from the verified JWT and passes it in;
//               these helpers NEVER widen scope (`narrowCasinoScope` only permits a
//               requested casino equal to the authorised one). Products consume this
//               via `@/lib/platform/evidence`, NOT another product's internals or raw
//               storage (Architecture Authority §7/§10).
//   Versioning: envelope shape is a contract; changes are additive or a new version.
//
// One framework for the four evidence domains (financial / session / player /
// machine): envelope assembly, filter + pagination validation, aggregate
// reconciliation, and CSV-injection-safe export. The edge function performs the
// JWT-scoped DB reads and calls into here — no metric is recomputed, and
// aggregates are supplied by the certified projection.
//
