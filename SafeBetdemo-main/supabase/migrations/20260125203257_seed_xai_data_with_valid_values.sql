/*
  # Seed Comprehensive XAI Data with Valid Constraint Values

  Seeds AI Reason Stacks, Recommendations, and Outcomes for players across all casinos.
*/

-- Clear existing data
TRUNCATE TABLE ai_intervention_outcomes CASCADE;
TRUNCATE TABLE ai_intervention_recommendations CASCADE;
DELETE FROM ai_reason_stacks;

-- Seed AI Reason Stacks for medium-high risk players
...
FROM players p
WHERE p.risk_score >= 35
ORDER BY p.risk_score DESC
LIMIT 800;

-- Seed AI Intervention Recommendations with valid intervention types
...
SELECT 
  ...
  CASE 
    ...
    WHEN RANDOM() > 0.6 THEN 'overridden'
    ELSE 'pending'
  END,
  CASE 
  ...
  CASE WHEN RANDOM() > 0.25 THEN now() - (RANDOM() * interval '5 days') ELSE NULL END,
...
SELECT 
  ...
  CASE 
    WHEN RANDOM() > 0.15 THEN 'risk_reduced'
  ...
  (3 + (RANDOM() * 10))::integer,
  CASE 
    WHEN RANDOM() > 0.25 THEN 'Positive - player acknowledged support'
