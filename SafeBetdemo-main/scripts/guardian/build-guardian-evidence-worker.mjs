...
//   products/guardian/dist-evidence-worker/index.js + package.json + guardian-evidence-worker.zip
...
const DIST = path.join(PKG, 'dist-evidence-worker');
const ENTRY = path.join(PKG, 'bin', 'guardian-evidence-worker.ts');

const sha = execSync('git rev-parse HEAD', { cwd: ROOT }).toString().trim();
const builtAt = new Date().toISOString();
const deploymentVersion = `guardian-evidence-worker-c7-${builtAt.replace(/[-:T]/g, '').slice(0, 12)}-${sha.slice(0, 7)}`;
...
try {
  execSync(`zip -j -q "${path.join(DIST, 'guardian-evidence-worker.zip')}" "${path.join(DIST, 'index.js')}" "${path.join(DIST, 'package.json')}"`, { cwd: DIST });
} catch {
  execSync(`powershell -NoProfile -Command "Compress-Archive -Force -Path '${path.join(DIST, 'index.js')}','${path.join(DIST, 'package.json')}' -DestinationPath '${path.join(DIST, 'guardian-evidence-worker.zip')}'"`);
}
