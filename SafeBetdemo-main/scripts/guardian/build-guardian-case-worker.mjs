...
//   products/guardian/dist-case-worker/index.js + package.json + guardian-case-worker.zip
...
const DIST = path.join(PKG, 'dist-case-worker');
const ENTRY = path.join(PKG, 'bin', 'guardian-case-worker.ts');

const sha = execSync('git rev-parse HEAD', { cwd: ROOT }).toString().trim();
const builtAt = new Date().toISOString();
const deploymentVersion = `guardian-case-worker-c6-${builtAt.replace(/[-:T]/g, '').slice(0, 12)}-${sha.slice(0, 7)}`;
...
try {
  execSync(`zip -j -q "${path.join(DIST, 'guardian-case-worker.zip')}" "${path.join(DIST, 'index.js')}" "${path.join(DIST, 'package.json')}"`, { cwd: DIST });
} catch {
  execSync(`powershell -NoProfile -Command "Compress-Archive -Force -Path '${path.join(DIST, 'index.js')}','${path.join(DIST, 'package.json')}' -DestinationPath '${path.join(DIST, 'guardian-case-worker.zip')}'"`);
}
