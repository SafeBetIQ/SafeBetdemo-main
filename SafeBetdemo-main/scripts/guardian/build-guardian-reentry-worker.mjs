#!/usr/bin/env node
// ─── SafeBet Guardian — re-entry worker Lambda build (ARCH-V4-C10) ───────────────
...
//   products/guardian/dist-reentry-worker/index.js + package.json + guardian-reentry-worker.zip
...
const DIST = path.join(PKG, 'dist-reentry-worker');
const ENTRY = path.join(PKG, 'bin', 'guardian-reentry-worker.ts');

const sha = execSync('git rev-parse HEAD', { cwd: ROOT }).toString().trim();
const builtAt = new Date().toISOString();
const deploymentVersion = `guardian-reentry-worker-c10-${builtAt.replace(/[-:T]/g, '').slice(0, 12)}-${sha.slice(0, 7)}`;
...
try {
  execSync(`zip -j -q "${path.join(DIST, 'guardian-reentry-worker.zip')}" "${path.join(DIST, 'index.js')}" "${path.join(DIST, 'package.json')}"`, { cwd: DIST });
} catch {
  execSync(`powershell -NoProfile -Command "Compress-Archive -Force -Path '${path.join(DIST, 'index.js')}','${path.join(DIST, 'package.json')}' -DestinationPath '${path.join(DIST, 'guardian-reentry-worker.zip')}'"`);
}
