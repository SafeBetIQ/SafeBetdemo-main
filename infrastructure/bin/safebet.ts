...
const alarmStack = new SafeBetDRAlarmStack(app, 'SafeBetDRAlarm', {
  ...
  crossRegionReferences: true,   // enables SSM-backed export of HealthCheckId to eu-west-1
  tags:                  baseTags,
});
...
const replicaStack = new SafeBetDRReplicaStack(app, 'SafeBetDRReplica', {
  ...
  // Cross-region reference: health check created in alarmStack (us-east-1).
  // alarmStack must have crossRegionReferences: true (set above).
  // CDK stores the ID in SSM Parameter Store in us-east-1 and reads it in eu-west-1.
