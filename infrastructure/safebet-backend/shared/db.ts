...
// ---------------------------------------------------------------------------
// Tenant-scoped query — sets app.casino_id for the duration of this client
// checkout so PostgreSQL RLS policies can enforce row-level isolation.
// The setting is session-level on the checked-out client; it is reset when
// the client is returned to the pool (pool.connect() checks out a fresh
// logical session each time).
// ---------------------------------------------------------------------------
export async function queryWithTenant<R extends QueryResultRow = QueryResultRow>(
  casinoId: string,
  sql: string,
  params?: unknown[],
  label = 'query',
  ...
  return withRetry(async () => {
    ...
    try {
      ...
      //   is_local=false → persists for the connection session (not just this tx)
      await client.query('SELECT set_config($1, $2, false)', ['app.casino_id', casinoId]);
...
// ---------------------------------------------------------------------------
// Tenant-scoped transaction — sets app.casino_id as a LOCAL setting so it
// is automatically cleared on COMMIT or ROLLBACK. All queries inside fn()
// execute within the same connection and see the same tenant context.
// ---------------------------------------------------------------------------
export async function withTenantTransaction<T>(
  ...
  return withRetry(async () => {
    ...
    try {
      ...
      // is_local=true → cleared automatically on transaction end
      await client.query('SELECT set_config($1, $2, true)', ['app.casino_id', casinoId]);
