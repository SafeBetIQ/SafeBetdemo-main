      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {NAV_ITEMS.map((item) =>
              item.children ? (
                ...
                >
                  <button className="flex items-center gap-1 px-4 py-2 text-sm text-gray-600 hover:text-gray-900 transition-colors rounded-md hover:bg-gray-50">
                    {item.label}
                    <ChevronDown className="h-3.5 w-3.5 text-gray-400" />
                  </button>
                  {openDropdown === item.label && (
                    <div className="absolute left-0 top-full pt-1 z-50">
                      <div className="bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden min-w-[240px] py-1">
                ...
                <Link
                  key={item.label}
                  href={item.href!}
                  className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900 transition-colors rounded-md hover:bg-gray-50"
                >
                  {item.label}
                </Link>
              )
          ...
          <div className="hidden lg:flex items-center gap-3">
            <Link href="/login">
              <Button variant="ghost" size="sm" className="text-gray-600 hover:text-gray-900 hover:bg-gray-100 font-medium">
                Sign In
              </Button>
            </Link>
            <Link href="/contact">
              <Button size="sm" className="bg-brand-600 hover:bg-brand-700 text-white font-medium px-5">
                Request Demo
              </Button>
            </Link>
          </div>
          ...
          <div className="px-6 py-4 space-y-1">
            ...
            <div className="pt-4 border-t border-gray-100 space-y-2">
              ...
              <Link href="/contact" onClick={() => setMobileMenuOpen(false)}>
                <Button size="sm" className="w-full bg-brand-600 hover:bg-brand-700 text-white">Request Demo</Button>
              </Link>
            </div>
          </div>
        </div>
